...
_SUPPORTED_QT_BINDINGS = ('PyQt6', 'PySide6', 'PyQt5', 'PySide2')
...
def startproject():
    ...
    _require_qt_bindings()
    app = prompt_for_value('App name', default='MyApp')
    user = getuser().title()
    author = prompt_for_value('Author', default=user)
    available_qt_bindings = [
        binding for binding in _SUPPORTED_QT_BINDINGS
        if _has_module(binding)
    ]
    if len(available_qt_bindings) == 1:
        python_bindings, = available_qt_bindings
    else:
        python_bindings = prompt_for_value(
            'Qt bindings',
            choices=available_qt_bindings or _SUPPORTED_QT_BINDINGS,
            default=_SUPPORTED_QT_BINDINGS[0]
        )
    ...
    # Do this before touching the file system, as it may fail:
    try:
        exec_method = _get_exec_method(python_bindings)
    except ImportError:
        raise FbsError(
            f'Could not load {python_bindings}. Are you able to run this ' \
            f'Python code?\n'
            f'    import {python_bindings}.QtWidgets'
        )
    mkdir('src')
...
def _get_exec_method(python_bindings):
    # Pyside2 and old PySide6 versions use exec_ whereas PyQt and new PySide6
    # versions use exec. (Eg.: PySide6 6.0.4 uses exec_ but 6.1.2 uses exec.)
    globals_ = dict()
    exec(f'from {python_bindings}.QtWidgets import QApplication', globals_)
    QApplication = globals_['QApplication']
    try:
        QApplication.exec
...
def _require_qt_bindings():
    if not any(_has_module(b) for b in _SUPPORTED_QT_BINDINGS):
        raise FbsError(
            "Couldn't find any Qt bindings. Maybe you need to:\n"
            "    pip install PySide6\n"
            "or\n"
            "    pip install PyQt6"
        )
